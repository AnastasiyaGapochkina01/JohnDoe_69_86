1) Создать пользователя `vally` с домашним каталогом и оболочкой `/bin/bash`; задать пользователю пароль
2) Создать ползователя `eve` без домашнего каталога (`useradd` без ключей)
3) Создать директорию `/home/eve`, дать пользователю `eve` права на него
4) Переименовать пользователя `vally` в `robot` и проверить, изменился ли UID
5) Создать группу `tms`
6) Добавить пользователя `robot` в группу `tms`
7) Создать директорию `/var/www/projects` и в ней
- файл `readme.md`
- файл `secret`
- директорию `bot-hub`
- файл  `bot-hub/app.py`
8) Установить права на файл `secret` так, чтобы только владелец мог читать и изменять его
9) Установить права на файл `readme.md` так, чтобы группа могла читать и изменять его (у владельца права не отбирать)
10) Уставновить права на `bot-hub` так, чтобы ни у одной категории пользователей не было бита `x`
11) Попробовать зайти в `bot-hub`; сделать `ls -l bot-hub`
12) Вернуть файлу `bot-hub/app.py` бит `x` для владельца и группы; повторить п.11
13) Назначить директоии `/var/www/projects` рекурсивно группой владельцев `tms`
14) Назначить владельцем файла `secret` пользователя `robot`, а владельцем `readme.md` - `eve`
15) Создать директорию `/opt/srv/backups` с владельцем `root` и группой владельцев `admins`; назнчить ей права, при которых
- владелец может делать все
- группа может делать все
- все остальные не могут ничего
16) Создать пользователя `backup`, добавить его в группу `admins`
17) Создать группу `web-users`
18) Переключиться на пользователя `backup` и проверить доступ к директории `/opt/srv/backups` создав там файл `test-bkp` с содержимым `Test backup`
19) Создать директорию `/opt/srv/nginx`, установить владельцем директории `www-data`, а группой `web-users`; назначить права на директорию
- владелец можем все
- группа может читать и исполнять
- все остальные не могут ничего
20) Создать пользователя `designer` и добавить его в группу `web-users`
21) Переключиться на пользователя `designer` и проверить доступ к директории `/opt/srv/nginx` - создать в ней файл `server.conf` с любым содержимым
Выйти из под пользователя `designer` и добавить группе `web-users` права на запись в `/opt/srv/nginx`; снова переключиться на `designer` и попробовать создать `server.conf` с любым содержимым
22) Создать сервисного (служебного) пользователя `backend` без возможности входа в систему
23) Переименовать пользователя `backend` в `apps-user`. Изменился ли UID?
